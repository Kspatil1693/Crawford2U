  ...
  authenticateUser: async function (request, response, error, next) {
    try {
      ...
      if (request && request.headers && request.headers.authorization) {
        ...
        const agentCheck = await AgentDetails.find({
      ...
      const clientCheck = await ClientDetails.find({
      ...
      console.log(error);
      jwt.verify(token, `${secretyKey}-${agentCheck[0].authToken}`, async function (error, data) {
        console.log(error);
        if (error) {
          return response.status(401).send({
            status: "FAILURE",
