...
exports.updateCategories = async (request, response, next) => {
  try {
    ...
    const categoriesCheck = await CategoriesDetails.findOne({
...
exports.deleteCategory = async (request, response, next) => {
  try {
    ...
    const categoriesCheck = await CategoriesDetails.deleteOne({
      name: name
    });
    if (categoriesCheck.length === 0) {
      return response.status(404).send({
...
exports.addSubCategories = async (request, response, next) => {
  try {
    ...
    const mainCategory = await CategoriesDetails.find({
      name: name
    });
...
exports.updateSubCategories = async (request, response, next) => {
  try {
    ...
    const subCategoriesCheck = await SubCategoriesDetails.findOne({
      subCategoriesName: subCategoriesName
    });
    if (subCategoriesCheck === 0) {
      return response.status(404).send({
...
exports.deleteSubCategory = async (request, response, next) => {
  try {
    ...
    const subCategoriesCheck = await SubCategoriesDetails.deleteOne({
      subCategoriesName: subCategoriesName
    });
    if (subCategoriesCheck.length === 0) {
      return response.status(404).send({
