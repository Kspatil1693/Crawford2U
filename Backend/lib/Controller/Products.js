...
exports.addProducts = async function (request, response, next) {
  try {
    ...
    const productCheck = await ProductDetails.find({
    ...
    const categoryData = await CategoriesDetails.find({
      name: name
    });
...
exports.updateProductDetails = async function (request, response, next) {
  try {
    ...
    const productCheck = await ProductDetails.findOne({
      productsName: productsName
    });
...
exports.deleteProducts = async function (request, response, next) {
  try {
    ...
    const productCheck = await ProductDetails.deleteOne({
      productsName: productsName
    });
    if (productCheck.length === 0) {
      return response.status(404).send({
